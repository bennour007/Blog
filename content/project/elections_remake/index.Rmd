...
de00 <- read_csv('https://raw.githubusercontent.com/bennour007/elections_2019/master/data/de19t0_clean.csv') %>% 
  mutate(prop_qs = (total_seats - q_seats)/total_seats * 100,
         prop_qs = round(prop_qs, 2)) %>% 
...
```{r echo=FALSE, message=TRUE, warning=TRUE}
de05 <- read_csv('https://raw.githubusercontent.com/bennour007/elections_2019/master/data/de19t5_clean.csv') %>% 
  mutate(prop_qs = (total_seats - q_seats)/total_seats * 100,
         prop_qs = round(prop_qs, 2)) %>% 
  mutate(party = fct_reorder(party, total_seats))
...
de00 %>% 
  ...
  select(party, total_seats.x, total_seats.y) %>% 
  mutate(total_seats.y = replace_na(total_seats.y, 0),
         diff = total_seats.y - total_seats.x) %>% 
  arrange(-diff) %>% 
  rename('seats at 0 %' = 'total_seats.x', 'seats at 5%' = 'total_seats.y') %>% 
  gt()
```
...
[here](https://bennour-med-hsin.shinyapps.io/e2019_dash/).
